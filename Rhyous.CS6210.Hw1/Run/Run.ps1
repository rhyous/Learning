...
Foreach ($disease in $diseases)
{
	...
	$analyzers += $endpoint;
    Start-Process .\Analyzer\Rhyous.CS6210.Hw1.OutBreakAnalyzer.exe -ArgumentList "n=`"Outbreak Analyzer: $($disease)`"","d=$($disease)","a=$($endpoint)","p=$($pubendpoint)","ns=$($endpointNameServer)"
	$port++;
} 
...
For ($i=0; $i -lt $analyzers.Length; $i++) {
    ...
    Start-Process .\HealthDistrict\Rhyous.CS6210.Hw1.HealthDistrict.exe -ArgumentList "n=`"Health District $($i)`"","d=$($hdendpoint)","p=$($publishers)","ns=$($endpointNameServer)"
   
    # Start 1 or 2 Simulators for every Health District
    $count = Get-Random -Minimum 1 -Maximum 2
    For ($j=0; $j -le $count; $j++) 
    {
        Start-Process .\Simulator\Rhyous.CS6210.Hw1.Simulator.exe -ArgumentList "n=`"Simulator $($i)`"","e=$($hdendpoint)","t=43200","u=30","ns=$($endpointNameServer)"
    }
}